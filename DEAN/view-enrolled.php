<?php 
    ...
    // Get the user_id from the URL (ensure it's always present)
    ...
    // Handle actions: accept/reject/undo
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {  // Ensure it's a POST request
        if (isset($_POST['undo'])) {
            // 'undo' action - set status to 'pre-enrolled'
            $status = 'pre-enrolled'; 

            // Update status to 'pre-enrolled' in the database
        ...
        elseif (isset($_POST['action']) && $_POST['action'] === 'reject') {
            ...
            if (mysqli_query($connection, $updateQuery)) {
                // Redirect after successful update
                header("Location: view-rejected.php?user_id=$user_id");
                exit;
            } else {
                die("Failed to update status: " . mysqli_error($connection));
            }
        }
        // Handle 'add_schedule_subject' action
        elseif (isset($_POST['action']) && $_POST['action'] === 'add_schedule_subject') {
            // Insert new schedule and subject
            ...
            if (mysqli_query($connection, $insertQuery)) {
                echo "<script>alert('Schedule and subject added successfully.'); window.location.href = window.location.href;</script>";
                exit; // Prevent further execution after redirect
            } else {
                echo "<script>alert('Error adding schedule and subject: " . mysqli_error($connection) . "');</script>";
            }
        }
    }
    ...
    SELECT 
        ...
        ce.semester,
        ce.course_name,
        ce.status,  -- Ensure status is included here
        r.file_grade,
        r.file_picture
    FROM 
        students s
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        // Check if 'status' key exists and assign it
...
<body>
    ...
    <script>
        ...
        if (<?php echo json_encode(isset($_POST['action']) && $_POST['action'] === 'add_schedule_subject'); ?>) {
